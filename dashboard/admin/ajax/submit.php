...
if (strtolower($param)=="checkmessage") {
	$tmp_id = '';
	$user_id = $_POST['to_id'];
	$sql = "SELECT * FROM tbl_chat where status_chat='started' and id_user='$user_id' order by id asc";
	$query = $db->query($sql);
	...
	if ($row==0) {
		...
		if ($result['status_chat']=='started') {
			$sql = "INSERT INTO chat_history (id_chat, isi_chat, id_pengirim, id_penerima, tanggal_chat )VALUES ('$id_chat', '$message', '$id_user', '$id_penerima', '$tanggal')";
